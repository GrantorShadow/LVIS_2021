...
#
#     This operation randomly cuts and paste's instances from one image and pastes it
#     in another image.
#
#      The CopyPaste transform steps are as follows:
#
#          1. Get two images from dataloader
#          2. Apply two random scaling transforms to the two images
#          3. Randomly select a set of subclasses from either of the randomly scaled images
#             (add class wise selection func for LVIS) to copy, then copy the masks
#          3. Paste the masks on the new image, which is randomly chosen from either of the two images
#
#     Args:
#
#     """
...
#         mask_2 = cv2.resize(mask, (w, h), interpolation=cv2.INTER_NEAREST)
#         mask_2 = np.asarray(mask_2, dtype=np.uint8)
#         sub_img2 = cv2.add(img_main, np.zeros(np.shape(img_main), dtype=np.uint8),
#                             mask=mask_2)
#
#         img_main = img_main - sub_img2 + cv2.resize(sub_img1, (img_main.shape[1], img_main.shape[0]),
#                                                        interpolation=cv2.INTER_NEAREST)
#
#         return img_main
#
#     def random_flip_horizontal(self, img, mask):
#         if np.random.random() < self.p:
#             img = img[:, ::-1, :]
#             mask = mask[:, ::-1]
#         return mask, img
#
#     def Large_Scale_Jittering(self, mask, img):
#         rescale_ratio = np.random.uniform(self.resize_scale[0], self.resize_scale[1])
#         h, w, _ = img.shape
#
#         # rescale the image
#         h_new, w_new = int(h * rescale_ratio), int(w * rescale_ratio)
#         img = mmcv.imresize(img, (w_new, h_new), interpolation='bilinear')
#         mask = mmcv.imresize(mask, (w_new, h_new), interpolation='bilinear')
#
...
#
#     def copy_paste(self, mask_src, mask_main, img_src, img_main, lsj=True):
#         # for main_mask, mask_source in zip(masks_main, masks_src):
#         mask_src, img_src = self.random_flip_horizontal(mask_src, img_src)
#         mask_main, img_main = self.random_flip_horizontal(mask_main, img_main)
#
#         if lsj:
#             mask_src, img_src = self.Large_Scale_Jittering(mask_src, img_src)
#             mask_main, img_main = self.Large_Scale_Jittering(mask_main, img_main)
#
#         img = self.img_add(img_src, img_main, mask_src)
#         mask = self.img_add(mask_src, mask_main, mask_src)
#
#         return mask, img
#
#     def get_rare_masks(self, masks, labels):
#         """
#         Returns the masks based on class labels
#         Args:
#             masks:
#             labels:
#
...
#         rare = []
#
#         for idx, vals in enumerate(zip(masks, labels)):
#             mask, label = vals
#             if label not in rare:
#                 np.delete(masks, idx)
#                 np.delete(labels, idx)
#
#         return masks, labels
#
#     def annToMask(self, ann, img, label_check=True):
#         """
#         Convert annotation which can be polygons, uncompressed RLE, or RLE to binary mask.
#         :return: binary mask (numpy 2D array)
#         """
#         def get_colored_mask(masks):
#             for mask in masks:
#                 mask = im.fromarray(mask.astype(np.uint8), mode="P")
...
#         segm = ann['masks']
#         labels = ann['labels']
#         rare = []  # TODO add class values
#         masks = []
#
#         for seg, label in zip(segm, labels):
#             if label_check:
#                 if label not in rare:
#                     continue
#             if type(seg) == list:
...
#                 # try:
#                 masks.append(maskUtils.decode(rle) * label)
#                 # except:
#                 #     mask = maskUtils.decode(rle) * label
#             else:
#                 # rle
#                 masks = ann['masks']
#
#         # return rle
#         # common = [] # add the class values here
#         # rare = []
#         #
#         # if label_check:
#         #     for idx, label in enumerate(ann.get('labels')):
#         #         if label in rare:
#         #             ann['masks'][idx]
#         #
...
#         time.sleep(10)
#
#         # TODO get two images, from the wrapper instead of the single image given rn
#
#         # get the image and it's mask
#         img1 = mmcv.imread(results['img_prefix'] + results['img_info']['filename'])
#         # masks1 = results.get('ann_info', [])
#         masks1 = results.get('gt_masks').masks
#         labels1 = results.get('gt_labels')
#
#         img2 = mmcv.imread(results['img_prefix'] + results['img_info']['filename'])
#         # masks2 = results.get('ann_info', [])
#         masks2 = results.get('gt_masks').masks
#         labels2 = results.get('gt_labels')
#
#         # convert the annotatations to masks
#         # # TODO merge with the lines above
#         # masks1 = self.annToMask(masks1, img1)
#         # masks2 = self.annToMask(masks2, img2)
#
#         # TODO sort the mask according to label and then return them
#         # if self.label_selection:
...
#         # if self.max_paste_objects > 0:
#         # masks1 = masks1[:self.max_paste_objects]
#         # masks2 = masks2[:self.max_paste_objects]
#
#         # choose the mask_main and mask src randomly
#         if np.random.random() < self.p:
#             img_main, masks_main = img1, masks1
#             img_src, masks_src = img2, masks2
#         else:
#             img_main, masks_main = img2, masks2
#             img_src, masks_src = img1, masks1
#
#         # img_main = [img1, img2][np.random.choice(2)]
#         # if img_main.all() == img1.all():
#         #     mask_main = masks1
#         #     mask_src = masks2
#         #     img_src = img2
#         # elif img_main.all() == img2.all():
#         #     mask_main = masks2
#         #     mask_src = masks1
#         #     img_src = img1
#
...
#
#
#         # # for key in [masks1, masks2]:
#         # mask_src = results.get(masks1[0])
#         # img_src = img1
#         #
#         # # TODO random selection bug fix
#         # #random choice main mask/img
#         # mask_main = results.get(np.random.choice([masks1[0], masks2[0]]))
#         # if mask_main == masks1:
#         #     mask_src = np.asarray(masks2)
#         #     img_src = img2
#         #     img_main = img1
#         # else:
#         #     mask_src = np.asarray(masks1)
#         #     img_src = img1
#         #     img_main = img2
#
#
...
#         # results['gt_bboxes'] = copypaste_bboxes
...
class CopyPaste:
    ...
    def random_flip_horizontal(self, mask, img):
        if np.random.random() < self.p:
            img = img[:, ::-1, :]
            mask = mask[:, ::-1]
        return mask, img

    def copy_paste(self, mask_src, mask_main, img_src, img_main):
        # for main_mask, mask_source in zip(masks_main, masks_src):
        mask_src, img_src = self.random_flip_horizontal(mask_src, img_src)
        mask_main, img_main = self.random_flip_horizontal(mask_main, img_main)
